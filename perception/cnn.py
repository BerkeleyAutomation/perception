...
class AlexNet(object):
    ...
    def _load(self):
        ...
        with self._graph.as_default():
            ...
            # form network
            self._input_node = tf.placeholder(
                tf.float32,
                (
                    self._batch_size,
                    self._im_height,
            ...
            self._feature_tensor = self.build_alexnet(
                weights, output_layer=self._feature_layer
            )
    ...
    def predict(self, image_arr, featurize=False):
        ...
        with self._graph.as_default():
            ...
            while i < num_images:
                ...
                end_ind = cur_ind + dim
                self._input_arr[:dim, :, :, :] = (
                    image_arr[cur_ind:end_ind, :, :, :] - self._mean
                )
                if featurize:
    ...
    def build_alexnet_weights(self):
        ...
        fc8W = tf.Variable(
            tf.truncated_normal(
                [fc8_in_size, fc8_out_size], stddev=0.01, seed=None
            )
        )
