...
class VirtualKinect2Sensor(CameraSensor):
    ...
    def frames(self):
        ...
        color_filename = os.path.join(
            self._path_to_images, "color_%d.png" % (self._im_index)
        )
        ...
        ir_filename = os.path.join(
            self._path_to_images, "ir_%d.npy" % (self._im_index)
        )
        ir_im = None
...
class Kinect2SensorFactory:
    ...
    def sensor(sensor_type, cfg):
        ...
        if sensor_type == "real":
            ...
            s = VirtualKinect2Sensor(cfg["image_dir"], frame=cfg["frame"])
        elif sensor_type == "bridged":
            s = KinectSensorBridged(quality=cfg["quality"], frame=cfg["frame"])
        else:
            raise ValueError(
                "Kinect2 sensor type %s not supported" % (sensor_type)
            )
        return s
...
def load_images(cfg):
    ...
    if "prestored_data" in cfg.keys() and cfg["prestored_data"] == 1:
        sensor = VirtualKinect2Sensor(
            path_to_images=cfg["prestored_data_dir"],
            frame=cfg["sensor"]["frame"],
        )
    else:
        sensor = Kinect2Sensor(
            device_num=cfg["sensor"]["device_num"],
            frame=cfg["sensor"]["frame"],
