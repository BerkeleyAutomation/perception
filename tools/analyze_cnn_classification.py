...
from autolab_core import YamlConfig, ClassificationResult, TensorDataset
...
def analyze_classification_performance(model_dir, config, dataset_path=None):
    ...
    experiment_log_filename = os.path.join(
        analysis_dir, "%s_analysis.log" % (dataset_name)
    )
    ...
    for i, label in enumerate(all_labels):
        ...
        plt.title(
            "Class %03d: %.3f%%"
            % (label, float(label_counts[label]) / dataset.num_datapoints),
            fontsize=3,
        )
        plt.axis("off")
    ...
    for split_name, indices in splits.iteritems():
        ...
        # predict
        if randomize:
            pred_probs, true_labels = cnn.evaluate_on_dataset(
                dataset, indices=indices, batch_size=predict_batch_size
            )
            ...
            for datapoint in dataset:
                im = ColorImage(
                    datapoint["color_ims"].astype(np.uint8)[:, :, :3]
                )
        ...
        plt.savefig(
            os.path.join(analysis_dir, "%s_confusion.pdf" % (split_name)),
            dpi=dpi,
        )
    ...
    plt.savefig(
        os.path.join(analysis_dir, "%s_precision_recall.pdf" % (dataset_name)),
        dpi=dpi,
    )
    ...
    plt.legend(handles, plt_labels, loc="best", fontsize=legend_font_size)
    plt.savefig(
        os.path.join(analysis_dir, "%s_roc.pdf" % (dataset_name)), dpi=dpi
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune a Classification CNN trained on ImageNet on a custom image dataset using TensorFlow"
    )
    parser.add_argument(
        "model_dir",
        type=str,
        default=None,
        help="directory of the model to use",
    )
    parser.add_argument(
        "config_filename",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="directory of the dataset to evaluate on",
    )
    ...
    analyze_classification_performance(
        model_dir, config, dataset_path=dataset_path
    )
