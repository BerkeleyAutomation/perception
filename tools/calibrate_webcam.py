...
parser.add_argument("chessboard_size", type=int, nargs=2, help="number of chessboard corners")
parser.add_argument("calib_folder", type=str, help="path to folder of calibration images")
parser.add_argument("--device", type=int, help="camera device number, if taking images")
...
if len(images) == 0:
    ...
    cam = RgbdSensorFactory.sensor("webcam", {"frame": "webcam", "intrinsics": None, "device_id": device})
    cam.start()
    for i in range(15):
...
ci = CameraIntrinsics("webcam", mtx[0, 0], mtx[1, 1], mtx[0, 2], mtx[1, 2], mtx[0, 1], *gray.shape[:2])
