...
from autolab_core import YamlConfig, TensorDataset
...
def finetune_classification_cnn(config):
    ...
    # init generator
    train_generator_filename = os.path.join(
        model_dir, "train_preprocessor.pkl"
    )
    ...
    if os.path.exists(train_generator_filename):
        ...
        train_generator = TensorDataGenerator(
            num_classes=num_classes, **data_aug_config
        )
        ...
        fit_start = time.time()
        train_generator.fit(
            dataset, x_names, y_name, indices=train_indices, **preproc_config
        )
    ...
    # init iterator
    train_iterator = train_generator.flow_from_dataset(
        dataset,
        x_names,
        y_name,
    ...
    val_iterator = val_generator.flow_from_dataset(
        dataset,
        x_names,
        y_name,
        indices=val_indices,
    ...
    latest_model_ckpt = ModelCheckpoint(
        latest_model_filename, period=model_save_period
    )
    best_model_ckpt = ModelCheckpoint(
        best_model_filename, save_best_only=True, period=model_save_period
    )
